  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #7f1d1d; padding: 24px; border-radius: 12px 12px 0 0;">
        <h2 style="color: #ffffff; margin: 0;">🔥 Nueva Reserva — Don Bife</h2>
        <p style="color: #fca5a5; margin: 4px 0 0 0; font-size: 14px;">Las mejores carnes y parrillas de Piura</p>
      </div>
      <div style="background: #fff; border: 1px solid #fee2e2; border-top: none; border-radius: 0 0 12px 12px; padding: 24px;">
        ...
        <p style="color: #9ca3af; font-size: 12px; margin-top: 24px; border-top: 1px solid #f5f5f4; padding-top: 16px;">Enviado automáticamente por el Agente IA de Don Bife 🔥</p>
      </div>
    </div>
  `;

  await getResend().emails.send({
    from: "Don Bife IA <onboarding@resend.dev>",
    to: [restaurantEmail],
    subject,
    html,
  });
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #fee2e2; border-radius: 12px;">
        <h2 style="color: #7f1d1d;">Reserva Cancelada — Don Bife</h2>
        ...
        <p style="color: #9ca3af; font-size: 12px;">Enviado automáticamente por el Agente IA de Don Bife 🔥</p>
      </div>
    `,
  });
}
