  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #7f1d1d; padding: 24px; border-radius: 12px 12px 0 0;">
        <h2 style="color: #ffffff; margin: 0;">Nuevo Pedido — Sr. Polaco</h2>
        <p style="color: #fca5a5; margin: 4px 0 0 0; font-size: 14px;">Barbería Clásica — Urb. Santa Isabel, Piura</p>
      </div>
      <div style="background: #fff; border: 1px solid #fee2e2; border-top: none; border-radius: 0 0 12px 12px; padding: 24px;">
        ...
        <p style="color: #9ca3af; font-size: 12px; margin-top: 24px; border-top: 1px solid #f5f5f4; padding-top: 16px;">Enviado automáticamente por el Agente IA de Sr. Polaco</p>
      </div>
    </div>
  `;

  await getResend().emails.send({
    from: "Sr. Polaco IA <onboarding@resend.dev>",
    to: [restaurantEmail],
    subject,
    html,
  });
...
export async function sendReservationEmail(reservation: Reservation & { customerUsername?: string }) {
  ...
  const subject = `Nueva cita: ${reservation.customerName} — ${reservation.date} ${reservation.time}`;

  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #7f1d1d; padding: 24px; border-radius: 12px 12px 0 0;">
        <h2 style="color: #ffffff; margin: 0;">Nueva Cita — Sr. Polaco</h2>
        <p style="color: #fca5a5; margin: 4px 0 0 0; font-size: 14px;">Barbería Clásica — Urb. Santa Isabel, Piura</p>
      </div>
      <div style="background: #fff; border: 1px solid #fee2e2; border-top: none; border-radius: 0 0 12px 12px; padding: 24px;">
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr><td style="padding: 8px; border-bottom: 1px solid #f5f5f4;"><strong>Servicio</strong></td><td style="padding: 8px; border-bottom: 1px solid #f5f5f4;">${reservation.occasion || "No especificado"}</td></tr>
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #fee2e2; border-radius: 12px;">
        <h2 style="color: #7f1d1d;">Cita Cancelada — Sr. Polaco</h2>
        <p>El cliente <strong>${reservation.customerName}</strong> canceló su cita para el <strong>${reservation.date}</strong> a las <strong>${reservation.time}</strong> (${reservation.guests} persona(s)).</p>
        <p style="color: #9ca3af; font-size: 12px;">Enviado automáticamente por el Agente IA de Sr. Polaco</p>
      </div>
    `,
  });
}
